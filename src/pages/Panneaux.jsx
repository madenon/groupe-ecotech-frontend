...
import PanneauContent from "../actions/PanneauContent";
import PanneauAction from "../actions/PanneauAction";
import { axiosInstance } from "../lib/axios";
import PostMediaActionPanneau from "./PostMediaActionPanneau";
import { useAuth } from "../context/AuthContext";
import { platformIcons } from "../actions/SocialShared";
import { platformShareUrls } from "../actions/sharedConfig";
import PanneauxComments from "../actions/PanneauxComments";
const Panneaux = ({ post }) => {
  const { panneauId } = useParams();
  ...
  const { mutate: deletePost, isPending: isDeletingPost } = useMutation({
    mutationFn: async () => {
      await axiosInstance.delete(`/panneau/${post?._id}`);
    },
    onSuccess: () => {
  ...
  const { mutate: likePost, isPending: isLikingPost } = useMutation({
    mutationFn: async () => {
      await axiosInstance.post(`/panneau/${post._id}/like`);
    },
    onSuccess: () => {
      // Invalider les queries pour que l'état des posts soit mis à jour
      queryClient.invalidateQueries({ queryKey: ["posts"] });
 ...
 const { mutate: createComment, isPending: isAddingComment } = useMutation({
  mutationFn: async (newComment) => {
    await axiosInstance.post(`/panneau/${post._id}/comment`, {
      content: newComment,
    });
  },
  onSuccess: () => {
    queryClient.invalidateQueries({ queryKey: ["posts"] });
    queryClient.invalidateQueries({ queryKey: ["post", panneauId] });
  ...
  mutationFn: async (formData) => {
    return axiosInstance.put(`/panneau/${post._id}`, formData);
  },
  onSuccess: (res) => {
    const updatedPost = res.data.panneau; // Assurez-vous que vous accédez bien à `panneau`
    queryClient.invalidateQueries({ queryKey: ["posts"] });
    queryClient.invalidateQueries({ queryKey: ["post", post._id] });
    ...
    <div className="bg-white rounded-lg shadow mb-4 w-full max-w-full overflow-hidden">
      <div className="p-2">
        <div className="flex items-center justify-between mb-4">
          <div className="w-full overflow-auto flex items-center gap-3">
            <Link to={`/profile/${post?.author?.username}`}>
              <img
                src={post?.author?.profilePicture || "/lg.jpg"} // Fallback si pas d'image
                alt={post?.author?.name}
                className="size-10 shadow-md rounded-full"
                onError={(e) => (e.target.src = "/lg.jpg")} // Image de secours si l'image échoue
              />
            </Link>
        ...
        {isEditing ? (
          ...
          <>
            <PanneauContent post={post} createComment={handleAddComment} />
            <PostMediaActionPanneau post={post} />
